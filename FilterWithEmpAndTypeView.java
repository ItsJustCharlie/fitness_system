...
public class FilterWithEmpAndTypeView {
    static Scene getFilterActivity()  {
        ...
        filter.setOnAction(e-> {
            updateFilterTable(filteredTableView,empNameField, typeField);
        });

        HBox hBox= new HBox();
        hBox.getChildren().addAll(empNameField,typeField, filter);
    ...
    public static void updateFilterTable(TableView<Activity> filteredTableView,  TextField empNameField,TextField typeField){
        ...
        filteredTableView.setItems(MySqlActivityRepository.selectFilteredEmployees(name,type));
